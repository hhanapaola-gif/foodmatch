...
class WishlistController extends Controller
{
    ...
    public function addToWishlist(Request $request): JsonResponse
    {
        ...
        $wishlist = $this->wishlist->where('user_id', auth('api')->user()->id)->where('product_id', $request->product_id)->first();
    ...
    public function removeFromWishlist(Request $request): JsonResponse
    {
        ...
        if ($type == 'all') {
            ...
            if (!$wishlist) {
                return response()->json(['message' => translate('product_not_found')], 404);
            }
            $wishlist->delete();
        }
