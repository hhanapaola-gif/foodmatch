...
use App\Http\Controllers\Api\V1\SavedPlanController;
use App\Http\Controllers\Api\V1\PlanController;
use App\Http\Controllers\Api\V1\PlanCategoryController;
use App\Http\Controllers\Api\V1\DigitalPaymentController;

// Diagnostic: no localization middleware, no auth — just api group (SubstituteBindings + RateLimiter)
Route::post('auth/test-ping', function () {
    \Log::info('TEST-PING reached');
    return response()->json(['status' => 'ok', 'middleware' => 'api-group-only']);
});

Route::group(['namespace' => 'Api\V1', 'middleware' => 'localization'], function () {

    // FCM removed — stub endpoint to prevent 500 errors from legacy clients
    Route::post('fcm-subscribe-to-topic', fn() => response()->json(['status' => true, 'message' => 'notifications disabled']));
    ...
    Route::group(['prefix' => 'plans'], function () {
        ...
        Route::get('/{id}/recommendations', [PlanController::class, 'recommendations']);
        Route::get('/{id}', [PlanController::class, 'show']);
        Route::post('/{id}/order', [PlanController::class, 'order'])->middleware(['auth:api', 'is_active']);
        Route::post('/{id}/save', [SavedPlanController::class, 'save'])->middleware(['auth:api', 'is_active']);
        Route::delete('/{id}/unsave', [SavedPlanController::class, 'unsave'])->middleware(['auth:api', 'is_active']);
    });
    ...
    Route::group(['prefix' => 'customer', 'middleware' => ['auth:api', 'is_active']], function () {
        ...
        // FCM removed — stub to prevent errors from legacy clients
        Route::put('cm-firebase-token', fn() => response()->json(['status' => true, 'message' => 'notifications disabled']))->withoutMiddleware(['auth:api', 'is_active']);
        ...
        Route::get('saved-plans', [SavedPlanController::class, 'index']);
        Route::get('saved-plan-ids', [SavedPlanController::class, 'savedIds']);
