...
Route::match(['GET', 'OPTIONS'], '/image-proxy', function () {
    $allowedOrigins = [
        'https://foodmatch-f2561.web.app',
        'https://foodmatch-f2561.firebaseapp.com',
    ];
    $origin = request()->header('Origin', '');
    $corsHeaders = in_array($origin, $allowedOrigins) ? [
        'Access-Control-Allow-Origin'  => $origin,
        'Access-Control-Allow-Methods' => 'GET, OPTIONS',
        'Access-Control-Allow-Headers' => 'Authorization, Content-Type, Accept, X-localization, branch-id',
    ...
    try {
        ...
        if ($response->failed() || $response->status() === 404) {
            return response('', 404, $corsHeaders);
        }

        return response($response->body(), $response->status(), $corsHeaders)
...
Route::post('/subscribeToTopic', fn() => response()->json(['status' => true, 'message' => 'notifications disabled']));
