    ...
    <div class="content container-fluid">
        ...
        <div class="row g-2 mb-3">
            <div class="col-6 col-sm-3">
                <a href="{{ route('admin.plan-order.list') }}" class="dashboard--card {{ !$status ? 'active' : '' }}">
                    <h5 class="dashboard--card__subtitle">Todos</h5>
                    <h2 class="dashboard--card__title">{{ $counts['all'] }}</h2>
                </a>
            </div>
            <div class="col-6 col-sm-3">
                <a href="{{ route('admin.plan-order.list', ['status' => 'pending']) }}" class="dashboard--card {{ $status === 'pending' ? 'active' : '' }}">
                    <h5 class="dashboard--card__subtitle">Pendientes</h5>
                    <h2 class="dashboard--card__title">{{ $counts['pending'] }}</h2>
                </a>
            </div>
            <div class="col-6 col-sm-3">
                <a href="{{ route('admin.plan-order.list', ['status' => 'confirmed']) }}" class="dashboard--card {{ $status === 'confirmed' ? 'active' : '' }}">
                    <h5 class="dashboard--card__subtitle">Confirmados</h5>
                    <h2 class="dashboard--card__title">{{ $counts['confirmed'] }}</h2>
                </a>
            </div>
            <div class="col-6 col-sm-3">
                <a href="{{ route('admin.plan-order.list', ['status' => 'cancelled']) }}" class="dashboard--card {{ $status === 'cancelled' ? 'active' : '' }}">
        ...
        <div class="card mb-3">
            <div class="card-body py-2">
                <form method="GET" class="d-flex flex-wrap gap-2 align-items-end">
                    ...
                    <a href="{{ route('admin.plan-order.list') }}" class="btn btn-outline-secondary">Restablecer</a>
                </form>
            </div>
        </div>
                    ...
                    <table class="table table-hover table-borderless mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $order)
                                <tr>
                                    <td>
                                        <a href="{{ route('admin.plan-order.show', $order->id) }}" class="font-weight-bold">
                                            #{{ $order->id }}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1 align-items-center">
                                            <a href="{{ route('admin.plan-order.show', $order->id) }}"
                                               class="btn btn-sm btn-outline-primary" title="Ver detalle">
                                                <i class="tio-visible"></i>
                                            </a>
                                            <form method="POST"
                                                  action="{{ route('admin.plan-order.status', $order->id) }}"
