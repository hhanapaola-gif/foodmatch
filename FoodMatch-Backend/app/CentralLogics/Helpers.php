...
class Helpers
{
    ...
    /** FCM removed — no-op. */
    public static function send_push_notif_to_device($fcm_token, $data, $isDeliverymanAssigned = false): bool
    {
        return false;
    }

    /** FCM removed — no-op. */
    public static function send_push_notif_to_topic($data, $topic, $type, ?string $web_push_link = null, $isNotificationPayloadRemove = false): bool
    {
        return false;
    }
    ...
    public static function file_remover(string $dir, $image): bool
    {
        ...
        if (Storage::disk(config('filesystems.default'))->exists($dir . $image)) Storage::disk(config('filesystems.default'))->delete($dir . $image);
    ...
    {
        if(isset($data) && strlen($data) >1 && Storage::disk(config('filesystems.default'))->exists($path.$data)){
            return $src;
        }
        return $error_src;
    }
