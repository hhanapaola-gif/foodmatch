...
class BusinessSettingsController extends Controller
{
    ...
    /**
     * FoodMatch-specific replacement for the legacy restaurant-setup page
     * above: that page carries ~20 settings (currency, maintenance mode,
     * payment method combos, halal tag, etc.) FoodMatch doesn't use, which
     * only made the panel confusing. This is just company name/contact/logo.
     */
    public function companySettings(): Renderable
    {
        $companyName = $this->business_setting->where(['key' => 'restaurant_name'])->first()?->value ?? '';
        $phone = $this->business_setting->where(['key' => 'phone'])->first()?->value ?? '';
        $address = $this->business_setting->where(['key' => 'address'])->first()?->value ?? '';
        $logoName = Helpers::get_business_settings('logo');
        $logo = Helpers::onErrorImage($logoName, asset('storage/restaurant') . '/' . $logoName, asset('assets/admin/img/160x160/img2.jpg'), 'restaurant/');

        return view('admin-views.business-settings.company-settings', compact('companyName', 'phone', 'address', 'logo'));
    }

    public function companySettingsUpdate(Request $request): RedirectResponse
    {
        $request->validate([
        ...
        $this->InsertOrUpdateBusinessData(['key' => 'phone'], ['value' => $request['phone']]);
        $this->InsertOrUpdateBusinessData(['key' => 'address'], ['value' => $request['address']]);

        if ($request->hasFile('logo')) {
            $currentLogo = $this->business_setting->where(['key' => 'logo'])->first();
            $this->InsertOrUpdateBusinessData(['key' => 'logo'], [
                'value' => Helpers::update('restaurant/', $currentLogo->value ?? null, APPLICATION_IMAGE_FORMAT, $request->file('logo')),
            ]);
        }
    ...
    public function socialMediaStore(Request $request): JsonResponse
    {
        ...
        if ($validator->fails()) {
            return response()->json(['error' => 1, 'errors' => Helpers::error_processor($validator)]);
        }
    ...
    private function InsertOrUpdateBusinessData($key, $value): void
    {
        ...
        // Helpers::get_business_settings() caches the whole table forever —
        // without this, saved changes (like a new logo) never show up.
